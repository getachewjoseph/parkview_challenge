...
export default function AnalyticsScreen() {
  ...
  const generatePDFReport = async () => {
    ...
    try {
        ...
        <html>
          <head>
            <style>
              ...
              .risk-score { font-size: 24px; font-weight: bold; color: ${riskLevel.color}; }
              .section { margin-bottom: 25px; }
              .section-title { font-size: 18px; font-weight: bold; margin-bottom: 10px; color: #333; }
              .stat { margin: 5px 0; }
              .recommendation { margin: 5px 0; color: #333; }
              .footer { margin-top: 30px; text-align: center; color: #666; font-size: 12px; }
            </style>
          </head>
          <body>
            <div class="header">
              <h1>Fall Risk Analytics Report</h1>
              <p>Generated on ${currentDate}</p>
            </div>
            
            <div class="section">
              <div class="section-title">Overall Risk Assessment</div>
              <p>Risk Score: <span class="risk-score">${analyticsData.riskScore}% (${riskLevel.level})</span></p>
              <p>Based on falls, exercise, and screening data analysis</p>
            </div>
            
            <div class="section">
              <div class="section-title">Quick Statistics</div>
              <div class="stat">Falls in last 6 months: ${analyticsData.falls.length}</div>
              <div class="stat">Exercise weeks logged: ${analyticsData.exerciseLogs.length}</div>
              <div class="stat">Average minutes per week: ${analyticsData.exerciseLogs.length > 0 
                ? Math.round(analyticsData.exerciseLogs.reduce((sum, log) => sum + log.minutes, 0) / analyticsData.exerciseLogs.length)
                : 0}</div>
              <div class="stat">Screening reported falls: ${analyticsData.screening?.fall_count || 0}</div>
            </div>
            
            <div class="section">
              <div class="section-title">Risk Factors Analysis</div>
              <div class="stat">Recent falls detected: ${analyticsData.riskFactors.hasRecentFalls ? 'Yes' : 'No'}</div>
              <div class="stat">Multiple falls (>2): ${analyticsData.riskFactors.fallCount > 2 ? 'Yes' : 'No'}</div>
              <div class="stat">Low exercise weeks: ${analyticsData.riskFactors.lowExercise}</div>
              <div class="stat">Screening risk indicators: ${analyticsData.riskFactors.screeningRisk ? 'Yes' : 'No'}</div>
            </div>
            
            ${analyticsData.falls.length > 0 ? `
            <div class="section">
              <div class="section-title">Recent Falls</div>
              ${analyticsData.falls.slice(0, 5).map(fall => `
                <div class="stat">
                  ${formatDate(fall.fall_date)} - ${fall.location || 'Unknown location'}
                  ${fall.activity ? ` (${fall.activity})` : ''}
                </div>
              `).join('')}
            </div>
            ` : ''}
            ...
            <div class="section">
              ...
              ${analyticsData.riskScore >= 60 ? '<div class="recommendation">• Consider consulting with your healthcare provider about fall prevention strategies</div>' : ''}
              ${analyticsData.riskFactors.lowExercise > 2 ? '<div class="recommendation">• Increase weekly exercise to 150+ minutes for better fall prevention</div>' : ''}
              ${analyticsData.falls.length > 0 ? '<div class="recommendation">• Review fall locations and activities to identify patterns</div>' : ''}
              ${analyticsData.riskScore < 40 ? '<div class="recommendation" style="color: #4CAF50;">• Great job! Keep maintaining your current exercise routine</div>' : ''}
            </div>
            
            <div class="footer">
              <p>This report was generated by Parkview Fall Risk Analytics</p>
      ...
      const options = {
        html: html,
        fileName: `fall-risk-report-${currentDate.replace(/\//g, '-')}`,
        directory: 'Documents',
      };
